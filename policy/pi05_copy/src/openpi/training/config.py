                ...
                return _transforms.Group(
                    inputs=[
                        ...
                        _transforms.PadStatesAndActions(
                            ...
                            dual_arm=getattr(model_config, 'dual_arm', False),
                            actual_arm_dim=getattr(model_config, 'actual_arm_dim', 7),
                        ),
                    ],
                )
...
class LeRobotAlohaDataConfig(DataConfigFactory):
    ...
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Note: AlohaOutputs.dual_arm should be False because:
        # - For dual_arm configs, UnnormalizeDualArm already extracts 14D from 32D segmented output
        # - AlohaOutputs only needs to apply _encode_actions on the 14D output
        data_transforms = _transforms.Group(
            inputs=[aloha_policy.AlohaInputs(adapt_to_pi=self.adapt_to_pi)],
            outputs=[aloha_policy.AlohaOutputs(
                adapt_to_pi=self.adapt_to_pi,
                dual_arm=False,  # Always False - extraction handled by UnnormalizeDualArm
                actual_arm_dim=getattr(model_config, 'actual_arm_dim', 7),
            )],
        )
...
class TrainConfig:
    ...
    checkpoint_base_dir: str = "/data0/users/haoce/RoboTwin/policy/pi05_copy/checkpoints/"
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        batch_size=16,  # Reduced due to 3 VLMs
        fsdp_devices=4,
    ),
    #
    # RoboArena configs.
